# OpenBean developer install (Windows) — https://github.com/openbean/openbean
#
# This is the DEVELOPER install path: it clones the repo and brings up
# the local stack with hot-reload. It assumes Node.js 22+, git, and a
# container runtime are already installed. It is NOT the customer path.
#
# For the customer path — a production install that handles
# prerequisites, registers a system service, and opens the setup
# wizard in your browser — run instead:
#
...
#
# Usage:
#   irm https://openbean.xyz/dev-install.ps1 | iex
#
...
# V18: offer the HTTPS tunnel. The local instance is up and the
# browser is on the setup wizard. The next friction point is "how
# do my teammates on other machines reach this box?" A quick
# Cloudflare tunnel answers that with one command and no account.
Write-Host ""
Write-Host "------------------------------------------------------------------"
Write-Host ""
Write-Bold "OpenBean is up. One last step if you want remote access:"
Write-Host "  - The browser is on the setup wizard. Finish setup there."
Write-Host "  - When you're ready, run:  npm run tunnel"
Write-Host "  - It downloads cloudflared, starts a quick HTTPS tunnel,"
Write-Host "    and prints a https://*.trycloudflare.com URL your teammates"
Write-Host "    can use from any browser, anywhere."
Write-Host ""
$answer = Read-Host "Start the tunnel now? [Y/n]"
if ($answer -eq '' -or $answer -match '^[Yy]') {
  Write-Host ""
  Write-Bold "Starting the HTTPS tunnel..."
  npm run tunnel
} else {
  Write-Host ""
  Write-Host "  Skipped. Run 'npm run tunnel' when you want it."
